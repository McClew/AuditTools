...
foreach ($member in $members) {
    ...
    if ($allowList -notcontains $memberNameLower) {
        if ($deleteList -contains $memberNameLower) {
            ...
            # Use the SID or the Name string explicitly to avoid object mismatch
            Remove-LocalGroupMember -Group "Administrators" -Member $member.SID -ErrorAction SilentlyContinue
        }
    }
}
